...
prompt pure
alias gksu='pkexec env DISPLAY=$DISPLAY XAUTHORITY=$XAUTHORITY QT_QPA_PLATFORMTHEME="qt5ct"'
...
# run tmux
#if command -v tmux &> /dev/null && [ -n "$PS1" ] && [[ ! "$TERM" =~ screen ]] && [[ ! "$TERM" =~ screen-256color ]] && [[ ! "$TERM" =~ tmux ]] && [[ ! "$TERM" =~ tmux-256color ]] && [ -z "$TMUX" ]; then
# 	sleep 2
